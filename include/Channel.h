...
/**
 * 多Reactor模型上对应多路事件分发器
 * Channel理解为通道 封装了sockfd和其感兴趣的event 如EPOLLIN、EPOLLOUT事件 还绑定了poller返回的具体事件
 **/
class Channel : noncopyable
{
public:
    using EventCallback = std::function<void()>; // 读写事件回调函数对象类型定义
    using ReadEventCallback = std::function<void(Timestamp)>; // 读写事件回调函数对象类型定义
    ...
    // 防止当TcpConnection中的channel被析构掉时 channel还在执行回调操作
